<template>
  <div class="max-w-7xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <table v-else class="w-full text-left text-sm">
              ...
              <tbody>
                <tr v-for="order in orderbookWithCumulative.buy" :key="order.id" class="hover:bg-mushroom/20 transition-colors">
                  ...
                  <td class="py-2 text-elephant text-right text-xs">{{ order.cumulative.toFixed(8) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div>
            ...
            <table v-else class="w-full text-left text-sm">
              ...
              <tbody>
                <tr v-for="order in orderbookWithCumulative.sell" :key="order.id" class="hover:bg-mushroom/20 transition-colors">
    ...
    <div class="w-full lg:w-1/3">
      <div class="bg-mushroom/30 border border-elephant/20 rounded-lg p-6 shadow-sm sticky top-24">
        <h2 class="text-xl font-bold text-gunmetal tracking-tight mb-6">Place Limit Order</h2>
        
        <!-- Toggle Buy/Sell -->
        <div class="flex bg-white/60 rounded-lg p-1 mb-6 border border-elephant/20">
        ...
        <form @submit.prevent="placeOrder" class="space-y-4">
          <div>
            ...
            <div class="relative">
              ...
              <input v-model.number="price" type="number" step="0.01" min="0.01" class="w-full bg-white border border-elephant/30 rounded-lg pl-8 pr-4 py-3 text-gunmetal focus:outline-none focus:ring-2 focus:ring-thatch focus:border-transparent transition-all font-mono" required>
            </div>
          </div>
          ...
          <div v-if="error" class="text-thatch text-sm bg-thatch/10 p-3 rounded-lg border border-thatch/20 mb-4">
            {{ error }}
          </div>

          <button :disabled="submitting || price <= 0 || amount <= 0" type="button" @click="confirmOrder" :class="side === 'buy' ? 'bg-green-600 hover:bg-green-700' : 'bg-thatch hover:bg-thatch/90'" class="w-full text-white font-bold py-3.5 rounded-lg transition-all cursor-pointer disabled:opacity-50 shadow-md mt-2">
